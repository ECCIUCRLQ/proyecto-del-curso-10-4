...
void TcpServer::acceptConnectionRequest() {
  ...
  if (file == -1 || !this->listen) {
    if (!this->listen) {
      // Throw an error if connection is stopped but not by a signal
      throw std::runtime_error("could not accept client connection");
    } else {
      throw std::runtime_error("server killed by a signal");
    }
  } else {
    // Insert the socket in the list of active sockets
